...
{};h(a,"Maps/TilesProviders/OpenStreetMap.js",[],function(){class a{constructor(){this.subdomains=["a","b","c"];this.themes={Standard:{url:"https://tile.openstreetmap.org/{zoom}/{x}/{y}.png",minZoom:0,maxZoom:19},Hot:{url:"https://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png",minZoom:0,maxZoom:19},OpenTopoMap:{url:"https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png",minZoom:0,maxZoom:17,credits:'Map data: &copy; <a href="https://www.openstreetmap.org/copyright">\n                        OpenStreetMap</a> contributors, <a href="https://viewfinderpanoramas.org">SRTM</a> \n                        | Map style: &copy; <a href="https://opentopomap.org">OpenTopoMap</a> \n                        (<a href="https://creativecommons.org/licenses/by-sa/3.0/">CC-BY-SA</a>)'}};
...
z+"'.",!1)}l.subdomain&&e.subdomains&&-1!==e.subdomains.indexOf(l.subdomain)?t=l.subdomain:p(e.subdomains)&&-1!==g.url.indexOf("{s}")&&(t=B(e.subdomains&&e.subdomains[0],""),v("Highcharts warning: The Tiles Provider's Subdomain '"+l.subdomain+"' is not defined in the Provider definition - falling back to '"+t+"'.",!1));e.requiresApiKey&&(l.apiKey?g.url=g.url.replace("{apikey}",l.apiKey):(v("Highcharts warning: The Tiles Provider requires API Key to use tiles, use provider.apiKey to provide a token.",
!1),g.url=g.url.replace("?apikey={apikey}","")));l.url=g.url.replace("{s}",t);this.minZoom=g.minZoom;this.maxZoom=g.maxZoom;e=B(b.userOptions.credits&&b.userOptions.credits.text,"Highcharts.com "+B(g.credits,e.defaultCredits));b.credits?b.credits.update({text:e}):b.addCredits({text:e,style:B(null===(a=b.options.credits)||void 0===a?void 0:a.style,{})});c.projection.options.name!==f&&v("Highcharts warning: The set projection is different than supported by Tiles Provider.",!1)}else c.projection.options.name||
v("Highcharts warning: The set projection is different than supported by Tiles Provider.",!1);p(this.minZoom)&&m<this.minZoom?(m=this.minZoom,y=Math.pow(2,m),f=256/400.979322*Math.pow(2,x)/(256/400.979322*Math.pow(2,m)),w=256*f):p(this.maxZoom)&&m>this.maxZoom&&(m=this.maxZoom,y=Math.pow(2,m),f=256/400.979322*Math.pow(2,x)/(256/400.979322*Math.pow(2,m)),w=256*f);if(c.projection&&c.projection.def){c.projection.hasCoordinates=!0;r[m]||(r[m]=b.renderer.g().add(this.group));const h=(a,b,c,d)=>a.replace("{x}",
b.toString()).replace("{y}",c.toString()).replace("{zoom}",d.toString()).replace("{z}",d.toString());a=(a,e,k,f,m)=>{var C=a%y,g=e%y;C=0>C?C+y:C;g=0>g?g+y:g;!d[`${k}`].tiles[`${a},${e}`]&&l.url&&(g=h(l.url,C,g,k),d[k].loaded=!1,d[`${k}`].tiles[`${a},${e}`]=b.renderer.image(g,a*w-f,e*w-m,w,w).attr({zIndex:2,opacity:0}).on("load",function(){l.onload&&l.onload.apply(this);if(k===(0>c.zoom?0:Math.floor(c.zoom))||k===n.minZoom)d[`${k}`].actualTilesCount++,d[`${k}`].howManyTiles===d[`${k}`].actualTilesCount&&
(d[k].loaded=!0,n.isAnimating?n.redrawTiles=!0:(n.redrawTiles=!1,A(u)),d[`${k}`].actualTilesCount=0)}).add(r[k]),d[`${k}`].tiles[`${a},${e}`].posX=a,d[`${k}`].tiles[`${a},${e}`].posY=e,d[`${k}`].tiles[`${a},${e}`].originalURL=g)};f=c.pixelsToProjectedUnits({x:0,y:0});f=c.projection.def.inverse([f.x,f.y]);f={lon:f[0]-q,lat:f[1]};e=c.pixelsToProjectedUnits({x:b.plotWidth,y:b.plotHeight});e=c.projection.def.inverse([e.x,e.y]);e={lon:e[0]-q,lat:e[1]};if(f.lat>c.projection.maxLatitude||e.lat<-1*c.projection.maxLatitude)f.lat=
c.projection.maxLatitude,e.lat=-1*c.projection.maxLatitude;f=this.lonLatToTile(f,m);e=this.lonLatToTile(e,m);g=this.tileToLonLat(f.x,f.y,m);g=c.projection.def.forward([g.lon+q,g.lat]);t=c.projectedUnitsToPixels({x:g[0],y:g[1]});g=f.x*w-t.x;t=f.y*w-t.y;d[`${m}`]||(d[`${m}`]={tiles:{},isActive:!1,howManyTiles:0,actualTilesCount:0,loaded:!1});d[`${m}`].howManyTiles=(e.x-f.x+1)*(e.y-f.y+1);d[`${m}`].actualTilesCount=0;for(z=f.x;z<=e.x;z++)for(let b=f.y;b<=e.y;b++)a(z,b,m,g,t)}Object.keys(d).forEach(a=>
{Object.keys(d[a].tiles).forEach(e=>{if(c.projection&&c.projection.def){const k=256/400.979322*Math.pow(2,x)/(256/400.979322*Math.pow(2,parseFloat(a)))*256,g=d[a].tiles[Object.keys(d[a].tiles)[0]],{posX:h,posY:l}=d[a].tiles[e];if(p(h)&&p(l)&&p(g.posX)&&p(g.posY)){var f=this.tileToLonLat(g.posX,g.posY,parseFloat(a));f=c.projection.def.forward([f.lon+q,f.lat]);f=c.projectedUnitsToPixels({x:f[0],y:f[1]});const F=g.posX*k-f.x,p=g.posY*k-f.y;if(b.renderer.globalAnimation&&b.hasRendered){const b=Number(d[a].tiles[e].attr("x")),
c=Number(d[a].tiles[e].attr("y")),f=Number(d[a].tiles[e].attr("width")),g=Number(d[a].tiles[e].attr("height"));n.isAnimating=!0;d[a].tiles[e].attr({animator:0}).animate({animator:1},{step:(m,n)=>{d[a].tiles[e].attr({x:b+(h*k-F-b)*n.pos,y:c+(l*k-p-c)*n.pos,width:f+(Math.ceil(k)+1-f)*n.pos,height:g+(Math.ceil(k)+1-g)*n.pos})}},function(){n.isAnimating=!1;n.redrawTiles&&(n.redrawTiles=!1,A(u))})}else{if(n.redrawTiles||parseFloat(a)!==m||(d[a].isActive||parseFloat(a)===m)&&Object.keys(d[a].tiles).map(b=>
d[a].tiles[b]).some(a=>0===a.opacity))n.redrawTiles=!1,A(u);d[a].tiles[e].attr({x:h*k-F,y:l*k-p,width:Math.ceil(k)+1,height:Math.ceil(k)+1})}}}})})}else v("Highcharts warning: Tiles Provider not defined in the Provider Registry.",!1)}}update(){var {transformGroups:a}=this;const b=this.chart.mapView,{provider:c}=arguments[0];a&&(a.forEach(a=>{0!==Object.keys(a).length&&a.destroy()}),this.transformGroups=[]);b&&!p(b.options.projection)&&c&&c.type&&(a=h[c.type])&&(a=new a,{initialProjectionName:a}=a,
b.update({projection:{name:a}}));super.update.apply(this,arguments)}}E.defaultOptions=A(u.defaultOptions,{states:{inactive:{enabled:!1}}});D(r,"beforeMapViewInit",function(a){var b=(this.options.series||[]).filter(a=>"tiledwebmap"===a.type)[0];({geoBounds:a}=a);if(b&&b.provider&&b.provider.type&&!b.provider.url){b=h[b.provider.type];if(p(b)){b=new b;({initialProjectionName:b}=b);if(this.options.mapView)if(a){const {x1:c,y1:d,x2:h,y2:n}=a;this.options.mapView.recommendedMapView={projection:{name:b,
parallels:[d,n],rotation:[-(c+h)/2]}}}else this.options.mapView.recommendedMapView={projection:{name:b},minZoom:0};return!1}v("Highcharts warning: Tiles Provider not defined in the Provider Registry.",!1)}return!0});a.registerSeriesType("tiledwebmap",E);"";return E});h(a,"masters/modules/tiledwebmap.src.js",[a["Core/Globals.js"],a["Maps/TilesProviders/TilesProvidersRegistry.js"]],function(a,h){a.TilesProvidersRegistry=h})});
