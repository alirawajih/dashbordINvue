...
a)});b=g(!0,{attributes:{markerWidth:20,markerHeight:20,refX:0,refY:0,orient:"auto"}},b,c);b=this.definition(b);b.id=a;return b}const {defaultMarkers:b}=l,{addEvent:m,defined:f,extend:r,merge:g,uniqueKey:q}=h,x=[],y=e("marker-end"),D=e("marker-start"),B="rgba(192,192,192,"+(k.svg?1E-4:.002)+")";class v extends d{static compose(b,f){h.pushUnique(x,b)&&m(b,"afterGetContainer",c);h.pushUnique(x,f)&&(f.prototype.addMarker=a)}constructor(a,b,c){super(a,b,c,"shape");this.type="path"}toD(){var a=this.options.d;
...
e.push(["L",Math.round(d.x),Math.round(d.y)],["L",Math.round(b.x),Math.round(b.y)]));return e}}const {merge:c}=h;class a extends k{linkPoints(){super.linkPoints();this.linkRetracementsPoints()}linkRetracementsPoints(){const b=this.points,c=b[0].y-b[3].y,e=b[1].y-b[2].y,d=b[0].x,g=b[1].x;a.levels.forEach((f,m)=>{const h=b[0].y-c*f;f=b[1].y-e*f;m=this.options.typeOptions.reversed?a.levels.length-m-1:m;this.startRetracements=this.startRetracements||[];this.endRetracements=this.endRetracements||[];this.linkRetracementPoint(m,
d,h,this.startRetracements);this.linkRetracementPoint(m,g,f,this.endRetracements)})}linkRetracementPoint(a,c,e,d){const b=d[a],f=this.options.typeOptions;b?(b.options.x=c,b.options.y=e,b.refresh()):d[a]=new l(this.chart,this,{x:c,y:e,xAxis:f.xAxis,yAxis:f.yAxis})}addShapes(){a.levels.forEach(function(a,c){const {backgroundColors:b,lineColor:d,lineColors:g}=this.options.typeOptions;this.initShape({type:"path",d:e(c),stroke:g[c]||d},c);0<c&&this.initShape({type:"path",fill:b[c-1],strokeWidth:0,d:e(c,
!0)})},this)}addLabels(){a.levels.forEach(function(a,e){const b=this.options.typeOptions;a=this.initLabel(c(b.labels[e],{point:function(a){return l.pointToOptions(a.annotation.startRetracements[e])},text:a.toString()}));b.labels[e]=a.options},this)}}a.levels=[0,.236,.382,.5,.618,.786,1];a.prototype.defaultOptions=c(k.prototype.defaultOptions,{typeOptions:{reversed:!1,height:2,backgroundColors:"rgba(130, 170, 255, 0.4);rgba(139, 191, 216, 0.4);rgba(150, 216, 192, 0.4);rgba(156, 229, 161, 0.4);rgba(162, 241, 130, 0.4);rgba(169, 255, 101, 0.4)".split(";"),
...
a);b.resizeX+=d;b.resizeY+=c;b.redraw(!1,!0)}}}});return d.types.measure=B});p(d,"masters/modules/annotations-advanced.src.js",[d["Core/Globals.js"],d["Extensions/Annotations/Annotation.js"]],function(d,l){d.Annotation=l;l.compose(d.Chart,d.Pointer,d.SVGRenderer)})});
